...
public class TextureAtlas
{
    ...
    public static TextureAtlas FromAtlasData(ContentManager content, TextureAtlasData atlasData)
    {
        TextureAtlas atlas = new TextureAtlas();
        atlas.Texture = content.Load<Texture2D>(atlasData.Texture);
        foreach (var region in atlasData.Regions)
        {
            atlas.AddRegion(region.Name, region.X, region.Y, region.Width, region.Height);
        }

        foreach (var animationFrame in atlasData.Animations)
        {
            List<TextureRegion> frames = new List<TextureRegion>();
            foreach (var frame in animationFrame.Frames)
            {
                TextureRegion region = atlas.GetRegion(frame.Region);
                frames.Add(region);
            }
            TimeSpan delay = TimeSpan.FromMilliseconds(animationFrame.Delay);

            Animation animation = new Animation(frames, delay);
            atlas.AddAnimation(animationFrame.Name, animation);
        }
