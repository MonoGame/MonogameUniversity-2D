...
public class Slime
{
    ...
    private void HandleInput()
    {
        ...
        if (Core.Input.MoveUp())
        {
            potentialNextDirection = -Vector2.UnitY;
        }
        else if (Core.Input.MoveDown())
        {
            potentialNextDirection = Vector2.UnitY;
        }
        else if (Core.Input.MoveLeft())
        {
            potentialNextDirection = -Vector2.UnitX;
        }
        else if (Core.Input.MoveRight())
        {
            potentialNextDirection = Vector2.UnitX;
        }
    ...
    public void Draw()
    {
        ...
        foreach (SlimeSegment segment in _segments)
        {
            ...
            // larping between its "at" and "to" position by the movement
            // offset lerp amount
    ...
    public Circle GetBounds()
    {
        ...
        Circle bounds = new Circle(
            (int)(pos.X + _sprite.HalfWidth),
            (int)(pos.Y + _sprite.HalfHeight),
            (int)_sprite.HalfWidth
        );
